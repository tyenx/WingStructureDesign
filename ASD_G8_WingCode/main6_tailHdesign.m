...
des_para.xcg_tailH = -(0.494-0.4); % Distance from center of gravity to shear center (positive in front of the shear center), in terms of chord length
des_para.xhg_tailH = -(0.7-0.4); % Distance from elevator hinge to shear center (positive in front of the shear center), in terms of chord length

% Wing root section horizontal tail airfoil geometry parameters (NACA-0009 9% smoothed)
wingbox_width_root = 1.298; % Wing box width (chordwise), i.e., chord length, in meters
wingbox_height_root = 0.212; % Wing box height (thickness), in meters
Dsection_length_root = 0.394; % D-section skin circumferential half perimeter, in meters
Dsection_radius_root = 0.43;  % D-section skin curvature radius, in meters
...
%% Tail design
% Section division
section_length = [2.675,2.675]; % Length of each section (spanwise), in meters
section_critical_x = [0,2.675]; % Distance from the root of the tail to the maximum stress position of each section, in meters
section_chord = [2.36,1.888]; % Chord length at the maximum stress position of each section, in meters
section_chord_root = [2.36,1.888]; % Chord length on the root side of each section, in meters
section_chord_tip = [1.888,1.416]; % Chord length on the tip side of each section, in meters

% Initial design parameters
stringer_thickness = 2e-3; % Thickness of stringers, in meters
stringer_web_height = 35e-3; % Height of stringer web, in meters
stringer_flange_web_ratio = [0.3,0.3]; % Ratio of stringer web height to flange width
stringer_number = [10,9]; % Number of stringers
spar_flange_width_front = [30,30]*1e-3; % Front spar flange width, in meters
spar_flange_thickness_front = 15e-3; % Front spar flange thickness, in meters
spar_flange_width_rear = [30,30]*1e-3; % Rear spar flange width, in meters
spar_flange_thickness_rear = 15e-3; % Rear spar flange thickness, in meters

% Design for each section separately
side = 'left'; % Choose which side to compute
for i = 1:2
    % Calculate loads
